...
class MCommercials extends CI_Model {
  ...
  function get_all($user_id = NULL) {
    $data = array();
    $this->db->select('commercials.*, users.name as user_name');
    $this->db->from('commercials');
    $this->db->join('users', 'users.id = commercials.user_id');
    if ($user_id) {
      $this->db->where('commercials.user_id', $user_id);
    }
